...
int main(int argc, char const *argv[])
{
    ...
    mapped[0] = (void *)addr;
    cprintf("mmap step2 addr(0x80000000) = 0x%x.\n", addr);

    addr = 0x80001000;
    assert(mmap(&addr, size, MMAP_WRITE) == 0);
    mapped[1] = (void *)addr;
    cprintf("mmap step2 addr(0x80001000) = 0x%x.\n", addr);

    cprintf("mmap step2 ok.\n");

    addr = 0x80001800;
    assert(mmap(&addr, 100, 0) != 0);
    cprintf("mmap step3 addr(0x80001800) = 0x%x.\n", addr);
    ...
    for (i = 0; i < 128; i ++)
    {
        // 上面 mmap 映射创建好之后，这里的写操作会触发缺页中断，然后分配物理内存
        //cprintf("i1 i=%d\n",i);
        buffer[i] = (char)(i * i);
    }
