 ...
 * data space.
 */
static inline int get_stack_long(struct proc_struct *task, int offset)
{
    unsigned char *stack;

    stack = (unsigned char *)(thiscpu->cpu_ts.ts_esp0);
    stack += offset;
    return (*((int *)stack));
}
 ...
 * data space.
 */
static inline int put_stack_long(struct proc_struct *task, int offset, unsigned long data)
{
    unsigned char *stack;
 ...
 */
static unsigned long get_long(struct proc_struct *tsk, unsigned long addr)
{
    unsigned long page;
 ...
 */
static void put_long(struct proc_struct *tsk, unsigned long addr, unsigned long data)
{
...
repeat:
    ...
    if (!(page & PTE_W))
    {
        if (!(page & PAGE_COW))
            readonly = 1;
        do_wp_page(PTE_W | PTE_P, addr, tsk, 0);
        goto repeat;
    }
    /* this is a hack for non-kernel-mapped video buffers and similar */
    if (page >= high_memory)
    ...
    *(unsigned long *)page = data;
    if (readonly)
    {
        *(unsigned long *)pte &= ~(PTE_W | PAGE_COW);
        invalidate();
    }
}
 ...
 */
static int read_long(struct proc_struct *tsk, unsigned long addr, unsigned long *result)
{
 ...
 */
static int write_long(struct proc_struct *tsk, unsigned long addr, unsigned long data)
{
    unsigned long low, high;
    ...
    switch (request)
    {
        ...
        case PTRACE_POKEUSR: /* write the word at location addr in the USER area */
            ...
            /* We need to be very careful here.  We implicitly
               want to modify a portion of the proc_struct, and we
