...
int dev_e1000_init(struct device *devptr)
{
    ...
    cprintf("EP_BRC= %02x:%02x:%02x:%02x:%02x:%02x dvminor=%d dvnum=%d\n", sr[0], sr[1], sr[2], sr[3], sr[4], sr[5], devptr->dvminor, devptr->dvnum);

    cprintf("eth[%d].baddr: %02x:%02x:%02x:%02x:%02x:%02x \n", devptr->dvminor, etptr->etbcast[0], etptr->etbcast[1], etptr->etbcast[2], etptr->etbcast[3], etptr->etbcast[4], etptr->etbcast[5]);
...
int dev_e1000_open(struct device *dev, uint32_t open_flags, uint32_t arg2)
{
    return dev->dvnum;
}

int dev_e1000_close(struct device *dev)
{
    return OK;
}

int dev_e1000_ioctl(struct device *dev, int op, void *arg1, void *arg2)
{
    return -E_UNIMP;
}

int dev_e1000_write(struct device *dev, char *buff, int len)
{
...
int dev_e1000_read(struct device *dev, char *buff, int len)
{
    int result = e1000_recv((uint8_t *)buff, len);
    return result;
}

int e1000_read(struct iobuf *iob)
{
    int result = e1000_recv(iob->io_base, iob->io_len);
    return result;
}

int e1000_write(struct iobuf *iob)
{
    int result = e1000_transmit(iob->io_base, iob->io_len);
    return result;
}

int dev_e1000_io(struct device *dev, struct iobuf *iob, bool write)
{
    int result;
...
static void e1000_device_init(struct device *dev)
{
    //struct etblk  *etptr;
...
void dev_init_e1000(void)
{
    ...
    e1000_device_init(device_vop_info(node));
