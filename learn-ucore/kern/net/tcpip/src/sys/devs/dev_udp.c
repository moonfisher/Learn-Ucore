...
int udp_init(struct device *dev)
{
    return dginit(dev);
    //return OK;
}

int udp_open(struct device *dev, uint32_t open_flags, uint32_t arg2)
{
...
int udp_close(struct device *dev)
{
...
int udp_io(struct device *dev, struct iobuf *iob, bool write)
{
    panic("udp_io not be implemented\n");
    return -E_UNIMP;
}

int udp_ioctl(struct device *dev, int op, void *arg1, void *arg2)
{
...
void dev_init_udp()
{
    ...
    udp_device_init((device_vop_info(node)));
