...
int tcp_init(struct device *dev)
{
    return tcpinit(dev);
    //return OK;
}

int tcp_open(struct device *dev, uint32_t open_flags, uint32_t arg2)
{
...
int tcp_close(struct device *dev)
{
    return tcpclose(dev);
...
int tcp_io(struct device *dev, struct iobuf *iob, bool write)
{
    panic("tcp_io not be implemented\n");
    return -E_UNIMP;
}

int tcp_ioctl(struct device *dev, int op, void *arg1, void *arg2)
{
...
void dev_init_tcp()
{
    ...
    tcp_device_init(device_vop_info(node));
