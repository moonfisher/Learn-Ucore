 ...
 */
static struct inode *lookup_ffs_nolock(struct ffs_fs *ffs, TCHAR * path, struct ffs_inode *parent)
{
...
static int ffs_create_inode(struct ffs_fs *ffs, struct ffs_disk_inode *din, const char *path, struct inode **node_store, struct ffs_inode *parent)
{
    ...
    {
        inode_init(node, ffs_get_ops(din->type), info2fs(ffs, ffs));
...
int ffs_load_inode(struct ffs_fs *ffs, struct inode **node_store, TCHAR *path, struct ffs_inode *parent, const char *name)
{
    ...
    ffs_set_links(ffs, ffs_vop_info(node));
